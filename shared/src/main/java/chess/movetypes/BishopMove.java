...
public class BishopMove extends Moves{
    ...
    public BishopMove(ChessBoard board, ChessPosition current, ChessGame.TeamColor color){
        this.moves = new ArrayList<>();
        getBishopMoves(this.moves, board, current, color);
    }

    protected static void getBishopMoves(ArrayList<ChessMove> moves, ChessBoard board, ChessPosition current, ChessGame.TeamColor color){
        ...
        for (int i=1; i<9; i++) {
            if (tracker1) {
                ChessPosition position1 = new ChessPosition(current.getRow() + i, current.getColumn()+i);
                if (Moves.inBounds(position1) && (board.getPiece(position1) == null || board.getPiece(position1).getTeamColor() != color)) {
            ...
            if (tracker2) {
                ChessPosition position2 = new ChessPosition(current.getRow() - i, current.getColumn()-i);
            ...
            if (tracker3) {
                ChessPosition position3 = new ChessPosition(current.getRow() + i, current.getColumn()-i);
            ...
            if (tracker4) {
                ChessPosition position4 = new ChessPosition(current.getRow() - i, current.getColumn()+i);
